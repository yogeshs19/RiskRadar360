...
# streamlit_app.py — RiskRadar360 (keys-fixed, form UX)
...
def assess_tab(tab_name: str):
    ...
    with st.form(key=key(tab_name, \"Form\")):
        ...
        def ask(question, default_yes: bool, L: int, I: int, rname: str, cat: str, mitigation: str):
            default_index = 0 if default_yes else 1  # \"Yes\" when default_yes else \"No\"
        ...
        # Group by 'group' and show in expanders
        grouped = {}
        for (cat, rname, question, risk_when_true, L, I, mitigation, group) in TAB_DEF[tab_name]:
            grouped.setdefault(group, []).append((cat, rname, question, risk_when_true, L, I, mitigation))
