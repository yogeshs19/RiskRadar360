...
# streamlit_app.py — RiskRadar360 (v2)
...
def k(tab: str, name: str) -> str:
    return f"{tab}_{name}".replace(" ", "_")
...
# ---------- Risk Definitions (category, risk_name, question, risk_when_true, L, I, mitigation, category_group) ----------
L10N_RISKS = [
...
def assess_tab(tab_name: str):
    hero()
    with st.form(key=k(tab_name, "form")):
        st.subheader(f"{tab_name} Project Risk Assessment")
        c1, c2, c3 = st.columns([2,2,1])
        project = c1.text_input("Project Name", key=k(tab_name, "project"))
        version = c2.text_input("Version / Release", key=k(tab_name, "version"))
        date_str = datetime.date.today().strftime("%Y-%m-%d")
        c3.markdown(f"**Date**  \n{date_str}")
        assessor = st.text_input("Assessor (optional)", "", key=k(tab_name, "assessor"))
        notes = st.text_area("Notes (optional)", "", key=k(tab_name, "notes"))
        st.markdown("---")

        # Group questions by category group inside expanders
        risk_rows = []
        categories = {}

        def ask(question, default_yes: bool, L: int, I: int, rname: str, cat: str, mitigation: str):
            idx = 0 if default_yes else 1
            ans = st.radio(question, ["Yes","No"], index=idx, horizontal=True, key=k(tab_name, rname))
        ...
        # Build expanders by group
        groups = {}
        for (cat, rname, question, risk_when_true, L, I, mitigation, group) in TAB_DEF[tab_name]:
            groups.setdefault(group, []).append((cat, rname, question, risk_when_true, L, I, mitigation))

        for grp, items in groups.items():
            with st.expander(f"▸ {grp}", expanded=True):
                for (cat, rname, question, risk_when_true, L, I, mitigation) in items:
                    # if risk_when_true => default radio "No" meaning it's a risk if "Yes"
    ...
    if submitted:
        ...
        c1.metric("Overall Rating", overall)
        c2.metric("High-Risk Items (≥7)", high_count)
        c3.metric("Total Score", total_score)

        st.markdown("---")
        cA, cB = st.columns(2)
        with cA: render_heatmap(risk_rows)
        with cB: render_radar(categories)
...
with tab_l10n:  assess_tab("L10n")
with tab_locops: assess_tab("LocOps")
with tab_general: assess_tab("General")
